...
void data::setFile(const path& relative_path)
{
	if (exists(relative_path)) {
		this->file_path = relative_path;
		setFile_last_modified(to_time_t(std::filesystem::last_write_time(relative_path)));

		path wordlistPath = file_path;
		path mapName = wordlistPath.stem();
		mapName += ".dat";
		this->map_path = std::filesystem::current_path() / "db" / "maps" / mapName;
	}
	else {
		string error_description = ("Could not open the file - '" + relative_path.string()) + "'";
...
{
	try {
		path filepath = std::filesystem::current_path() / "db" / "maps";
		if (!exists(filepath))
...
bool data::storedIsValid(data* const& map, bool debug)
{
	...
	time_t wordlist_last_modified = to_time_t(std::filesystem::last_write_time(wordlistPath));
	time_t map_last_modified = to_time_t(std::filesystem::last_write_time(mapPath));
