...
int main(int argc, char* argv[])
{
	...
	while (lookingForFile) {
		...
		{
			cout << "Cannot open " << std::filesystem::current_path() / path_to_wordlist << endl;
			EXIT_FAILURE;
		}

		map->setFile(path_to_wordlist);
		cout << "Loading " << std::filesystem::current_path() / path_to_wordlist << "..." << endl;
		clock_t t = clock();
		try {
